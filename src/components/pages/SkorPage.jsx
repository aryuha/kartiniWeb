...
export default function SkorPage() {
    ...
    const getGrade = (s) => {
        if (s >= 480) return { grade: 'S', color: '#22c55e', label: t('skorPerfect') }
        if (s >= 400) return { grade: 'A', color: '#3b82f6', label: t('skorGreat') }
        if (s >= 300) return { grade: 'B', color: '#f59e0b', label: t('skorGood') }
        if (s >= 150) return { grade: 'C', color: '#f97316', label: t('skorOk') }
    ...
    const summaryItems = [
        ...
        { label: t('skorLabelBonus'), value: `+${bonus}`, color: '#22aa44' },
        { label: t('skorLabelTime'), value: formatTime(timeElapsed), color: '#7799bb' },
                        ...
                        <div style={sk.colLeft}>
                            ...
                            <div style={sk.scoreCard}>
                                ...
                                <div style={sk.scoreTextWrap}>
                                    ...
                                    <p style={sk.scoreSubMessage}>
                                        {isWin ? t('skorStabilized') : t('skorComingSoon')}
                                    </p>
                                </div>
                            </div>
                                    ...
                                    {leaderboard.length === 0 ? (
                                        ...
                                        leaderboard.map((entry, i) => (
                                            <div key={i} style={{
                                                ...
                                                {/* Time */}
                                                <span style={sk.lbTime}>
                                                    {Math.floor((entry.completion_time || 0) / 60)}:{String((entry.completion_time || 0) % 60).padStart(2, '0')}
                                                </span>
...
const sk = {
    ...
    lbTime: {
        fontFamily: "'Orbitron',monospace",
        fontSize: 8, color: '#99aabc', flexShrink: 0,
    },
    lbScore: {
        fontFamily: "'Orbitron',monospace",
